...
欢迎开始你的 AI 转型之旅！在接下来的课程中，你将从一个 C++ 系统开发者，成长为一个能**独立设计和实现 AI 应用系统**的工程师。

这不是一门教你训练模型的课——你不需要买 GPU，不需要懂数学推导。这是一门教你**用 AI 造产品**的课。
...
│     │    ReAct / 工具链 / 记忆管理      │
├─────────────────────────────────────────┤
│  4F │ 📚 RAG 知识增强                  │ ← 让 AI 有"外脑"
│     │    Embedding / 向量检索 / 重排序   │
├─────────────────────────────────────────┤
│  3F │ 🔌 模型接口层                    │ ← API 调用 / 流式输出
...
你已经建好了 1F 和 2F（C++ 框架 + 基础设施），甚至 3F 和部分 5F 也有了（多模型策略 + MCP 两段式推理）。接下来我们要把每一层补齐、加固、精装修。

## 🤔 为什么 C++ 开发者学 AI 有优势？

你可能觉得 AI 圈都是 Python，C++ 没用武之地。**大错特错**。

| 你已有的能力 | AI 领域的应用 | 别人缺的 |
|---|---|---|
| 内存管理 | 理解 KV Cache、显存优化 | Python 开发者不懂底层 |
| 多线程 | 并行推理、批处理调度 | 大部分人只会单线程 |
| 网络编程 | 推理服务、流式通信 | 很多人只会调 API |
| 设计模式 | Agent 框架设计 | 会用框架但不会设计 |
| 性能优化 | 推理加速、延迟优化 | 这是核心竞争力 |

**你的定位不是"会调 Python API 的人"，而是"能从底层设计 AI 系统的人"。**

## 📋 课程节奏

每节课 30 分钟到 1 小时，设计原则：

1. **大白话先行**：每个概念先用日常生活类比，5 秒建立直觉
2. **C++ 锚点**：新概念都和你已知的 C++ 知识关联
3. **动手为主**：每节课都有可运行的代码或实操步骤
4. **面试导向**：每个知识点都给面试标准答案框架

## 🚀 现在开始

点击下一课，让我们从 Agent 升级开始——这是 ROI 最高的第一步。

> **小提示**：右上角有"📝 打开笔记"按钮，建议边学边记。好记性不如烂笔头。

\`\`\`javascript
// 试试互动实验室——运行一下看看效果
console.log("🎉 欢迎开始 AI 学习之旅！")
console.log("你已经有了 C++ 的坚实基础")
console.log("接下来，让我们给你的 Agent 装上翅膀 🚀")
\`\`\`
`,
      },
    ],
